...
GROQ_API_KEY ="enter the groq key here"
google_creds = "google json_content"
...
# Set up credentials at the start of your script
...
def main():
    ...
    """, unsafe_allow_html=True)
    # st.video("videos/qari part-1.mp4")
    ...
    with col1:
        recording_button = st.button(
            "🎙️ " + ("Stop Recording" if st.session_state['recording'] else "Start Recording")
        )
        ...
        if recording_button:
            ...
            if st.session_state['recording']:
                try:
                    filename = record_audio_with_progress()
                    ...
                    st.markdown("""
                        <div class="feedback-item" style="background-color: rgba(46, 204, 113, 0.1); border-left-color: #2ecc71;">
                            Recording completed successfully! ✅<br>Analysis will begin automatically...
                        </div>
                    """, unsafe_allow_html=True)
                    st.rerun()  # Trigger rerun to start analysis
                except Exception as e:
                    st.error(f"Recording failed: {str(e)}")
                    st.session_state['recording'] = False

    with col2:
        clear_button = st.button("🗑️ Clear Recording")
        if clear_button:
    ...
    # Analysis Section - Now automated
    if st.session_state['audio_file'] and os.path.exists(st.session_state['audio_file']) and not st.session_state['analysis_complete']:
        st.markdown("""
        <div class="card">
            <div class="card-header">
                <span style="font-size: 2rem;">🎵</span>
        ...
        with st.spinner("Analyzing your pronunciation..."):
            ...
            if transcription and validation_feedback:
                if "Validation Status: VALIDATED" in validation_feedback:
                    ...
                    # Generate and auto-play audio feedback
                    ...
                    if audio_path:
                        ...
                        """, unsafe_allow_html=True)
                        # Auto-play audio using HTML audio element
                        st.markdown(f"""
                        <audio autoplay>
